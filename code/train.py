...
parser.add_argument(
    "--lamda", type=float, default=0.5, help="weight to balance supervised loss"
)
parser.add_argument("--optimizer", type=str, default="SGD", help="optimizer")
parser.add_argument("--conf_thresh", type=float, default=0.95, help="conf_thresh")
parser.add_argument("--temperature", type=float, default=1, help="temperature")
parser.add_argument("--s1", type=float, default=0.5, help="s1")
parser.add_argument("--s2", type=float, default=0.5, help="s2")
...
if __name__ == "__main__":
    ...
    if args.optimizer == "SGD":
        ...
        optimizer = optim.Adam(
            net.parameters(), lr=base_lr, weight_decay=0.0001, betas=(0.5, 0.999)
        )
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, (
            ...
            pred_x, pred_u_w, pred_u_w_fp, pred_u_w_fp_mid, pred_u_w_fp_strong = net(
                torch.cat((img_x, img_u_w)),
                need_fp=True,
            )
            if args.temperature != 1:
                pred_u_w = pred_u_w / args.temperature
                pred_u_w_fp = pred_u_w_fp / args.temperature
                pred_u_w_fp_mid = pred_u_w_fp_mid / args.temperature
                pred_u_w_fp_strong = pred_u_w_fp_strong / args.temperature

            conf_u_w_fp = pred_u_w_fp.softmax(dim=1).max(dim=1)[0]
            conf_u_w_fp_mid = pred_u_w_fp_mid.softmax(dim=1).max(dim=1)[0]

            pred_u_s1, pred_u_s2 = net(torch.cat((img_u_s1, img_u_s2))).chunk(2)
            ...
            loss_u_w_fp = dice_loss(
                pred_u_w_fp.softmax(dim=1)[:, 1, :, :, :],
                masks,
                ignore=ignores,
            )
 
            loss_u_w_fp_mid = dice_loss(
                pred_u_w_fp_mid.softmax(dim=1)[:, 1, :, :, :],
                masks,
                ignore=ignores,
            )

            loss_u_w_fp_strong = dice_loss(
                pred_u_w_fp_strong.softmax(dim=1)[:, 1, :, :, :],
                masks,
                ignore=ignores,
            )
            ...
            loss_T_TA1_dice = loss_u_w_fp
            loss_T_TA1 = loss_T_TA1_dice

            # ---------------------
            # TA1 To TA2
            # ---------------------

            loss_TA1_TA2_ignore = (conf_u_w_fp < conf_thresh).float()
            loss_TA1_TA2_dice = dice_loss(
                pred_u_w_fp_mid.softmax(dim=1)[:, 1, :, :, :],
                pred_u_w_fp.argmax(dim=1) == 1,
                ignore=loss_TA1_TA2_ignore,
            )

            loss_TA1_TA2 = loss_TA1_TA2_dice

            # ---------------------
            # TA2 To STU
            # ---------------------
            loss_TA2_STU_ignore = (conf_u_w_fp_mid < conf_thresh).float()

            loss_TA2_STU_dice = dice_loss(
                pred_u_w_fp_strong.softmax(dim=1)[:, 1, :, :, :],
                pred_u_w_fp_mid.argmax(dim=1) == 1,
                ignore=loss_TA2_STU_ignore,
            )

            loss_TA2_STU = loss_TA2_STU_dice

            loss_u_w_pert = (1 - eta) * (
                loss_u_w_fp + loss_u_w_fp_mid + loss_u_w_fp_strong
            ) / 3.0 + eta * (loss_T_TA1 + loss_TA1_TA2 + loss_TA2_STU) / 3.0

            loss = (loss_x + loss_u_s1 * args.s1 + loss_u_s2 * args.s2 + loss_u_w_pert) / (2.0 + args.s1 + args.s2)
            ...
            writer.add_scalar("loss/loss_u_w_pert", loss_u_w_pert, iter_num)
