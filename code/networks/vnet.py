...
class VNet(nn.Module):
    def __init__(
        ...
        self.pert = PertDropout(p=pert_prob)
    ...
    def forward(self, input, turnoff_drop=False, need_fp=False):
        ...
        if need_fp:
            ...
            outs = self.decoder([torch.cat(feats_all) for feats_all in all_zip], no_drop=True)
...
class PertDropout(nn.Module):
    def __init__(self, p=0.5):
        ...
        self.p = p
        # self.dropouts = [
        #     nn.Dropout2d(p).cuda(), # Weak
        #     nn.AlphaDropout(p).cuda(),  # To
        #     nn.FeatureAlphaDropout(p).cuda(),  # Strong
        # ]
        self.dropouts = [
            nn.Dropout3d(p * 0.5).cuda(),  # Weak
            nn.Dropout3d(p).cuda(),  # To
            nn.Dropout3d(p * 1.5).cuda(),  # Strong
        ]
...
if __name__ == "__main__":
    ...
    print(macs, params)
    print(
        "VNet have {} paramerters in total".format(
